...
#define ADD_1ST_DCFD 0x103C     // DCFD Control for DPP-PSD
#define ADD_PSD_QTHR 0x1044
...
#define ADD_1ST_LATE 0x1n6C     // latency for coincidence
...
#define ADD_FIO_CTRL 0x811C	// Front Panel I/O Control
 ...
 )
{
  ...
  if( isActive == 0 ) {
    ...
    case 1:
      ...
      break;
    case 2:
      conntype = CAEN_DGTZ_USB_A4818_V2718;
      break;
    case 3:
      conntype = CAEN_DGTZ_USB_A4818_V3718;
      break;
    case 5:
    ...
    error = (int)CAEN_DGTZ_OpenDigitizer
      (
       conntype,
       link_number,
       connection_node,
	...
	boardInfo.AMC_FirmwareRel[i] = boardInfoFromLib.AMC_FirmwareRel[i];
      ...
      if( verboseDebug ) {
	std::cout << std::dec << "ModelName:\t"	<< boardInfo.ModelName		<< std::endl;
	std::cout << std::dec << "Model:\t"	<< boardInfo.Model		<< std::endl;
	std::cout << std::dec << "Channels:\t"	<< boardInfo.Channels		<< std::endl;
	std::cout << std::dec << "FormFactor:\t"	<< boardInfo.FormFactor		<< std::endl;
	std::cout << std::dec << "FamilyCode:\t"	<< boardInfo.FamilyCode		<< std::endl;
	std::cout << std::dec << "ROC FW rel:\t"	<< boardInfo.ROC_FirmwareRel 	<< std::endl;
	std::cout << std::dec << "AMC FW rel:\t"	<< boardInfo.AMC_FirmwareRel	<< std::endl;
	std::cout << std::dec << "Serial no.:\t"	<< boardInfo.SerialNumber	<< std::endl;
	std::cout << std::dec << "ADC_NBits:\t"	<< boardInfo.ADC_NBits		<< std::endl;
	
	// Time Bomb downcounter (expiration time DPP firmware)
	...
	std::cout << "Mins left:\t" << mins_left << std::endl;
      ...
      boardInfo.vers = 0;
      if( vers >= 0x30 && vers <= 0x39 ){
	boardInfo.vers         = 720;
	boardInfo.nsPerSample  =   4;
	boardInfo.nsPerTimetag =   8;
      }
      if( vers >= 0x60 && vers <= 0x62 ) {
	  boardInfo.vers         = 751;
	  boardInfo.nsPerSample  =   1;
	  boardInfo.nsPerTimetag =   8;
      }
      if( vers >= 0x90 && vers <= 0x90 ) {
	boardInfo.vers         = 724;
	boardInfo.nsPerSample  =  10;
	boardInfo.nsPerTimetag =   8;
      }
      /*
	V1730B  - 16 ch, 14 bit, 500 MS/s, 5.12MS/ch
	V1730C  -  8 ch, 14 bit, 500 MS/s,  640kS/ch
	V1730D  -  8 ch, 14 bit, 500 MS/s, 5.12MS/ch
	V1730   - 16 ch, 14 bit, 500 MS/s,  640kS/ch
	V1730SB - 16 ch, 14 bit, 500 MS/s, 5.12MS/ch
	V1730SC -  8 ch, 14 bit, 500 MS/s,  640kS/ch
	V1730SD -  8 ch, 14 bit, 500 MS/s, 5.12MS/ch
	V1730S  - 16 ch, 14 bit, 500 MS/s,  640kS/ch
	
	0xC0 = V1730/VX1730/DT5730/N6730;
	0xC1 = V1730B/VX1730B/DT5730B/N6730B;
	0xC2 = V1730C/VX1730C;
	0xC3 = V1730D/VX1730D;
	0xC4 = V1730S/VX1730S/DT5730S/N6730S;
	0xC5 = V1730BS/VX1730BS/DT5730BS/N6730BS;
	0xC6 = V1730CS/VX1730CS;
	0xC7 = V1730DS/VX1730DS;
      */
      if( vers >= 0xc0 && vers <= 0xc7 ){
	boardInfo.vers         = 730;
	...
	WriteRegister( 0x809C, 1 );
      ...
      if( vers >= 0xd0 && vers <= 0xd0 ){
	boardInfo.vers         = 720;
	boardInfo.nsPerSample  =   4;
	boardInfo.nsPerTimetag =   8;
      }
      /*
	  V1725B  - 16 ch, 14 bit, 250 MS/s, 5.12MS/ch
	  V1725C  -  8 ch, 14 bit, 250 MS/s,  640kS/ch
	  V1725D  -  8 ch, 14 bit, 250 MS/s, 5.12MS/ch
	  V1725   - 16 ch, 14 bit, 250 MS/s,  640kS/ch
	  V1725SB - 16 ch, 14 bit, 250 MS/s, 5.12MS/ch
	  V1725SC -  8 ch, 14 bit, 250 MS/s,  640kS/ch
	  V1725SD -  8 ch, 14 bit, 250 MS/s, 5.12MS/ch
	  V1725S  - 16 ch, 14 bit, 250 MS/s,  640kS/ch

	  0xF0 = V1725/VX1725/DT5725/N6725;
	  0xF1 = V1725B/VX1725B/DT5725B/N6725B;
	  0xF2 = V1725C/VX1725C;
	  0xF3 = V1725D/VX1725D;
	  0xF4 = V1725S/VX1725S/DT5725S/N6725S;
	  0xF5 = V1725BS/VX1725BS/DT5725BS/N6725BS;
	  0xF6 = V1725CS/VX1725CS;
	  0xF7 = V1725DS/VX1725DS;
	*/
	if( vers >= 0xf0 && vers <= 0xf7 ){
	  boardInfo.vers         = 725;
	  boardInfo.nsPerSample  =   4;
	  boardInfo.nsPerTimetag =  16;
	  // ADC Calibration
	  WriteRegister( 0x809C, 1 );
	}
	...
	if( verboseDebug ){
	  ...
	  std::cout << "nsPerTimetag " << 0+boardInfo.nsPerTimetag << ";";
	  std::cout << std::endl;
	}

	/*
	  Individual register. This kind of register has N instances, where N is the total number of
	  channels in the board. Individual registers can be written either in single mode (individual
	  setting) or broadcast (simultaneous write access to all channels). Read command must be
	  individual.
	  Single access can be performed at address 0x1nXY, where n is the channel number, while
	  broadcast write can be performed at the address 0x80XY.
	*/
	// ADD_1ST_FWRV, AMC Firmware Revision: 0x1n8C
	...
	/* DPP codes
	   ...
	   #define V1730_DPP_PHA_CODE	(0x8B) // In case of the DPP-PHA for x730 boards (anche x725?)
	...
	*/
	...
	error = (int)CAEN_DGTZ_GetDPPFirmwareType(handle,&boardInfo.dppVersion);
	switch (boardInfo.dppVersion){
	case CAEN_DGTZ_DPPFirmware_PHA:
	  std::cout << "Board is using DPP-PHA" << std::endl;
	  break;
	case CAEN_DGTZ_DPPFirmware_PSD:
	  std::cout << "Board is using DPP-PSD" << std::endl;
	  break;
	case CAEN_DGTZ_DPPFirmware_CI:
	  std::cout << "Board is using DPP-CI" << std::endl;
	  break;
	case CAEN_DGTZ_DPPFirmware_ZLE:
	  std::cout << "Board is using DPP-ZLE" << std::endl;
	  break;
	case CAEN_DGTZ_DPPFirmware_QDC:
	  std::cout << "Board is using DPP-QDC" << std::endl;
	  break;
	case CAEN_DGTZ_DPPFirmware_DAW:
	  std::cout << "Board is using DPP-DAW" << std::endl;
	  break;
	case CAEN_DGTZ_NotDPPFirmware:
	  std::cout << "Board is not using DPP firmware" << std::endl;
	  break;
	default:
	  std::cerr << "Firmware information not compatible with the CAENDigitizer library!" << std::endl;
	}
	/*
	  switch( (firmware & 0xFF00) >> 8 )
	  {
	  std::cout << "DPP firmware code: 0x" << ( (firmware & 0xFF00) >> 8 ) << std::endl;

	  case 0:
	  // Standard firmware code
	  // In case of waveform recording firmware
	  boardInfo.dppVersion = 0;
	  break;

	  case 0x80:
	  // In case of the DPP-PHA for x724 boards
	  boardInfo.dppVersion = 1;
	  break;

	  case 0x82:
	  // In case of the DPP-CI for x720 boards
	  boardInfo.dppVersion = 2;
	  break;

	  case 0x83:
	  // In case of the DPP-PSD for x720 boards
	  boardInfo.dppVersion = 3;
	  ConfigSetPSDMandatoryBits();
	  break;

	  case 0x84:
	  // In case of the DPP-PSD for x751 boards
	  boardInfo.dppVersion = 3;
	  ConfigSetPSDMandatoryBits();
	  break;

	  case 0x85:
	  // In case of the DPP-ZLE for x751 boards
	  break;

	  case 0x86:
	  // In case of the DPP-PSD for x743 boards
	  boardInfo.dppVersion = 3;
	  ConfigSetPSDMandatoryBits();
	  break;

	  case 0x87:
	  // In case of the DPP-QDC for x740D boards
	  break;

	  case 0x88:
	  // In case of the DPP-PSD for x730 boards

	  ConfigSetPSDMandatoryBits();
	  break;

	  case 0x8B:
	  // In case of the DPP-PHA for x730 boards (x725?)
	  boardInfo.dppVersion = 1;
	  break;

	  case 0x8C:
	  // In case of the DPP-ZLE for xx725/730 boards
	  break;

	  case 0x8D:
	  // In case of the DPP-daw for x725/x730 boards
	  break;
...
CAEN_DGTZ_DPPFirmware_t CAENDgtz::GetDPPVersion()
{
...
void CAENDgtz::Reset()
{
  ...
  if( boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ) {
    ConfigSetPSDMandatoryBits();
  }

  if( boardInfo.vers==724 && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PHA ) {
...
const uint16_t CAENDgtz::GetBoardRegID()
{
  return ReadRegister( ADD_VME_BDID, "GetBoardRegID" ) & 0xF;
}
...
uint32_t CAENDgtz::GetRegisterSpecificBits
(
 uint32_t reg_add,
 uint8_t bit_lower,
 uint8_t bit_upper,
...
void CAENDgtz::SetRegisterSpecificBits
(
 uint32_t reg_add,
 uint8_t bit_lower,
 uint8_t bit_upper,
...
/*
  This register manages the on-board fan speed in order to guarantee an appropriate cooling according to the internal
...
/*
  This function enables/disables the channels for the acquisition.
  Disabled channels don’t give any trigger and don’t participate to the event data.
...
/*
  ...
  [7:0]:
  Bit n corresponds to the trigger request from couple n that participates to the global trigger
  generation (n = 0,...,3 for DT, NIM and 8-channel VME boards; n = 0,...,7 for 16-channel VME
  boards).
  Options are:
  0 = trigger request does not participate to the global trigger generation;
  1 = trigger request participates to the global trigger generation.
  Couple n corresponds to the two consecutive channels 2n and 2n+1: couple 0 is channel 0
  and channel 1, couple 1 is channel 2 and channel 3, and so on. The trigger request from the
  couple can be programmed through register 0x1n84 to be the AND/OR/one of the channels.
  NOTE: in case of DT, NIM and 8-channel VME boards, only bits[3:0] are meaningful, while
...
/*
  GLOBAL TRIGGER MASK
  This register sets which signal can contribute to the global trigger generation.
  [26:24]: Majority Level
  Sets the majority level for the global trigger generation. For a level m, the
  ...
  NOTE: The majority level must be smaller than the number of channel enabled via bits[7:0]
  mask (or [3:0]).
*/
...
/*
  This function decides whether the trigger software should only be used to generate the acquisition trigger, only to
  generate the trigger output, or both.
*/
...
/*
  ...
  The acquisition mode, defined as a CAEN_DGTZ_AcqMode_t type.
  CAEN_DGTZ_SW_CONTROLLED = 0. Start and stop acquisition is issued by software command.
  CAEN_DGTZ_S_IN_CONTROLLED = 1. Acquisition starts when the external signal on S-IN connector sets high, while is stopped when it
  sets low. Instead of S-IN. GPI connector must be referred in case of Desktop/NIM boards.
  CAEN_DGTZ_FIRST_TRG_CONTROLLED = 2. Start is issued on the first trigger pulse (rising edge) on the TRG-IN connector. This pulse
  is not used as a trigger; actual triggers start from the second pulseon TRG-IN.
  The Stop acquisition must be SW controlled.
  Please refer to the digitizer documentation for details.
*/
...
/*
  This function decides whether the external trigger should only be used to generate the acquisition trigger, only to
  generate the trigger output, or both.
...
/*
  ...
  Options are:
  0 = Positive (the self-trigger is generated upon the input pulse over-threshold);
  1 = Negative (the self-trigger is generated upon the input pulse under-threshold).
*/
...
/*
  ...
  This bit enables a triangular (0<–>3FFF) test wave to be provided at the ADCs input for debug
  purposes.
  Options are:
  0 = disabled;
  1 = enabled.
...
/*
  Trigger Overlap Settng (default value is 0).
  When two acquisition windows are overlapped, the second trigger can be either accepted or
  rejected.
  Options are:
  0 = Trigger Overlapping Not Allowed (no trigger is accepted until the current acquisition
  window is finished);
  1 = Trigger Overlapping Allowed (the current acquisition window is prematurely closed by the
  arrival of a new trigger).
*/
...
/*
  ...
  A write access to this register causes a software clear.
  According to the BUFFER_CODE value written in the register, the number of buffers Nb is given by 2 elevato a BUFFER_CODE.
  Quindi es. con BufferBlockNumber=10, il numero di buffer nel quale risulta divisa la ram di ciascun canale è 1024.
*/
...
/*
  ...
  This function mainly applies to the digitizers running the waveform recording firmware for waves digitizing, since it
  manages the global trigger generation and its propagation through the TRG-OUT connector.

  For the x740 series, use the Set / GetGroupSelfTrigger function.

  Note: since 730, 725 and 743 families have even and odd channels paired, the user shouldn't call this function
  separately for the channels of the same pair, otherwise the second call will overwrite the setting of the first one. The
  user should instead call at maximum once for every pair with the relevant bits of the channelmask already set to the
  correct value.

  In case of Digital Pulse Processing (DPP) firmware, this function can also be used when it is required to manage the
  global trigger and/or propagate it out on TRG-OUT connector.

  Note: with DPP firmware, you should enable each channel you want to acquire to self-trigger on its own input. If you
  want to disable this option, you have to set DPPParams.selft = 0 (for DPP-PSD and DPP-CI).
  In case of DPP-PHA this option is not available in the library, while it is available via register write: set bit[24] = 0 of
  register 0x1n80 to enable the self-trigger of channel n, set it to 1 to disable the self-trigger.

  GLOBAL TRIGGER GENERATION da rivedere

  Probabilmente la funzione di Self-Trigger è enabled x default con DPP-PHA.
*/
...
/*
  UM1935 – CAENDigitizer Library User Manual Rev. 20

  This function sets the Trigger Threshold for a specific channel. The threshold is applied to the digital signal after the
  ADC and it is expressed in ADC counts. The user should take care of the DC offset adjust when converting the digital
  threshold in the corresponding voltage level on the analog input.

  For the x740 series, use the Set / GetGroupTriggerThreshold function. For DPP-QDC firmware on X740D family, use
  this function. For the others DPP firmware, use the SetDPPParameters function.

  UM5678 - 725-730 DPP-PHA User Manual rev. 1

  Threshold of the Trigger and Timing filter of the DPP-PHA algorithm. The threshold arms the RC-CR2 signal and the
  event is identified (trigger) on the RC-CR2 zero crossing.
...
void CAENDgtz::SetChannelTriggerThreshold( uint32_t channel, uint32_t threshold )
{
  ...
  // DPP-PHA Firmware. Non sembrano esserci differenze nei registri della 724 quindi commentata la linea seguente.
  //if( boardInfo.vers == 724 && boardInfo.dppVersion == 1 )
  if( boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PHA)
    {
      std::cout << "channel:" << channel << std::endl;
      std::cout << "Set threshold to:" << threshold << " lsb" << std::endl;
      // Trigger threshold value è [13:0]
      //SetRegisterSpecificBits( 0x106C + 0x0100 * channel, 0, 14, threshold, "SetChannelTriggerThreshold" );
      SetRegisterSpecificBits( 0x106C + 0x0100 * channel, 0, 13, threshold, "SetChannelTriggerThreshold" );
    }
  else if( boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD){
    SetRegisterSpecificBits( 0x1060 + 0x0100 * channel, 
			     0, 13, threshold, "SetChannelTriggerThreshold" );
    if( error != 0 ) {
    ...
    {
      // Va bene per firmware standard e DPP? Solo firmware standard?
...
uint32_t CAENDgtz::GetChannelTriggerThreshold( uint32_t channel )
{
  ...
  if( boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PHA )
    {
      return GetRegisterSpecificBits( 0x106C + 0x0100 * channel, 0, 13, "GetChannelTriggerThreshold" );
    }
  else if ( boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD )
    {
      return GetRegisterSpecificBits( 0x1060 + 0x0100 * channel, 0, 13, "GetChannelTriggerThreshold" );
    }
  else
    {
      // Va bene per firmware standard e DPP? Solo firmware standard?
...
/*
  Concerning the digitizers running the waveform recording firmware for waves digitizing, this function sets/gets the
  maximum number of events for each transfer. Regardless of the type of link, during a block transfer cycle, the digitizer
  stops the transfer after a predetermined number of events (or when the memory is empty).
...
/*
  POST TRIGGER
  The value of this register is used to set the number of post-trigger samples, that is the number of further samples that
  are written by the FPGA in the channel memory, when a trigger occurs, before to freeze the buffer.
  The number of post trigger samples is:
  Npost = PostTriggerValue x N + ConstantLatency
  where:
  ...
  ConstantLatency 	= constant number of samples added due to the latency associated to the trigger processing logic in
  the ROC FPGA. This value is constant, but the exact value may change between different firmware revisions.
*/
...
/*
  This function sets the size of the acquisition window, that is the number of samples that belong to it. Due to the way
  the samples are written into the memory (more samples are put in parallel), there is a specific granularity of the record
  length depending on the board model. For example, in the x720 series, the samples are written 4 by 4, hence the
  record length must be a multiple of 4. Please, refer to the User Manual of the specific board for the granularity value.
  The function accepts any value for the parameter size and then takes the closest value multiple of the granularity. The
  function GetRecordLength returns the exact value.
...
void CAENDgtz::SetChannelScopeSamples( uint32_t channel, uint32_t samples )
{
  ...
  /*
    channel (optional): To be used only with digitizers running DPP-PSD and DPP-PHA firmware. This parameter is not supported in DPP-CI firmware.
    Note: for the DPP-PSD and DPP-PHA firmware of 730 family (also 725?), the Set function is managed inside couples of channels, i.e. inside channel 0 and channel 1, channel 2 and channel 3, etc. It is only possible to set the record length for the even channel of the couple, then the same value is automatically applied for the odd channel of the couple.
    Con il firmware standard il valore impostato per un ch vale anche per tutti gli altri.
  */
...
uint32_t CAENDgtz::GetChannelScopeSamples( uint32_t channel )
{
  ...
  if(boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    data = GetRegisterSpecificBits( ADD_1ST_RELE + 0x100 * channel, 0, 13, "GetChannelScopeSamples" );
  }
  if( error != 0 ) {
...
/*
  ...
  CAEN_DGTZ_DPP_ACQ_MODE_Oscilloscope = 0L: enables the
  acquisition of the samples of the digitized waveforms.
  Note: Oscilloscope mode is not supported by DPP-PSD firmware of
  the 730 digitizer family and by DPP-QDC of x740D family.
  CAEN_DGTZ_DPP_ACQ_MODE_List = 1L: enables the acquisition of
  time stamps and energy values for each DPP firmware.
...
// DPP-PSD
// BOARD-CONFIGURATION, Address 0x8000, 0x8004 (BitSet), 0x8008 (BitClear)

void CAENDgtz::SetDataAutoFlush(int value)
{
  if(boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    if(value<2){
      error = (int)CAEN_DGTZ_WriteRegister( handle, 
					    ADD_DPP_CNFG + 0x0004 * (2 - value), 
					    1 );
    }else{
      error=-3;
    }
    if(error!=0)
      EmitError( "SetDataAutoFlush" );
  }   
}

uint32_t CAENDgtz::GetDataAutoFlush()
{
  if(boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD)
    return GetRegisterSpecificBits( ADD_DPP_CNFG , 0, 0, "GetDataAutoFlush" );
  else
    return 0;

}

void CAENDgtz::SetTriggerPropagation(int value)
{
  if(boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    if(value<2){
      error = (int)CAEN_DGTZ_WriteRegister( handle, 
...
uint32_t CAENDgtz::GetTriggerPropagation()
{
  if(boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD)
    return GetRegisterSpecificBits( ADD_DPP_CNFG, 2, 2, "GetTriggerPropation" );
  else
    return 0;

}

// DPP-PHA and DPP-PSD compliant
  ...
  if( value < 2 )
    {
      ...
      */
      ...
      //int ret = CAEN_DGTZ_SetDPP_VirtualProbe( handle, ANALOG_TRACE_1, CAEN_DGTZ_DPP_VIRTUALPROBE_Delta2 );
      int ret = CAEN_DGTZ_SetDPP_VirtualProbe( handle, ANALOG_TRACE_1, CAEN_DGTZ_DPP_VIRTUALPROBE_Input );
    }
  else
    {
      error = -3;
...
{
  /*
    This register contains general settngs for the board configuration.
    Address 0x8000, 0x8004 (BitSet), 0x8008 (BitClear)
    Mode R/W
    Attribute C
  */
  ...
  if( value < 3 )
    {
      switch( value )
        {
	  //[23:20] ”Peaking”, shows where the energy is calculated
	case 0:
	  {
	    error = (int)CAEN_DGTZ_WriteRegister( handle, ADD_DPP_CNFG + 0x0008, 15 << 20 );
	  }
	  break;
	...
	case 1:
	  {
	    error = (int)CAEN_DGTZ_WriteRegister( handle, ADD_DPP_CNFG + 0x0004, 1 << 20 );

	    error = (int)CAEN_DGTZ_WriteRegister( handle, ADD_DPP_CNFG + 0x0008, 7 << 21 );
	  }
	  break;
	...
	case 2:
	  {
	    error = (int)CAEN_DGTZ_WriteRegister( handle, ADD_DPP_CNFG + 0x0008, 1 << 20 );

	    error = (int)CAEN_DGTZ_WriteRegister( handle, ADD_DPP_CNFG + 0x0004, 1 << 21 );

	    error = (int)CAEN_DGTZ_WriteRegister( handle, ADD_DPP_CNFG + 0x0008, 3 << 22 );
	  }
	  break;
...
uint32_t CAENDgtz::GetEnabledScope()
{
  ...
  if( boardInfo.dppVersion == CAEN_DGTZ_NotDPPFirmware )	// STD firmware
    {
      return 1;
    }
  else				// DPP firmware
...
/*
  Number of pre trigger samples according to the formula Ns = N * 4, where Ns is the pre
  trigger and N is the register value. For example, write N = 5 to set 20 samples of pre trigger.
...
void CAENDgtz::SetChannelPreTrigger( uint32_t channel, uint32_t value )
{
  ...
  if( boardInfo.vers == 724 && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PHA)
  ...
  else
    {
      ...
      SetRegisterSpecificBits( 0x1038 + 0x0100 * channel, 0, 8, value, "SetChannelPreTrigger" );
    }
...
uint32_t CAENDgtz::GetChannelPreTrigger( uint32_t channel )
{
  ...
  if( boardInfo.vers == 724 && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PHA )
  ...
  else
    {
      ...
      data = GetRegisterSpecificBits( 0x1038 + 0x0100 * channel, 0, 8, "GetChannelPreTrigger" );
    }
...
void CAENDgtz::SetChannelPreGate( uint32_t channel, uint32_t value )
{
  ...
  SetRegisterSpecificBits( ADD_1ST_PGTE + 0x0100 * channel, 0, 8, value );
}
...
uint32_t CAENDgtz::GetChannelPreGate( uint32_t channel )
{
  ...
  return GetRegisterSpecificBits( ADD_1ST_PGTE + 0x0100 * channel, 0, 8 );
}
...
void CAENDgtz::SetChannelShortGate( uint32_t channel, uint32_t value )
{
  ...
  SetRegisterSpecificBits( ADD_1ST_SGTE + 0x100*channel, 
			   0, GetGateShortBitSize(), value, "SetChannelShortGate" );
...
uint32_t CAENDgtz::GetChannelShortGate( uint32_t channel )
{
  ...
  return GetRegisterSpecificBits( ADD_1ST_SGTE + 0x0100 * channel, 0, GetGateShortBitSize() );
}
...
void CAENDgtz::SetChannelLongGate( uint32_t channel, uint32_t value )
{
  ...
  SetRegisterSpecificBits( ADD_1ST_LGTE + 0x0100 * channel, 0, GetGateLongBitSize(), value );
...
uint32_t CAENDgtz::GetChannelLongGate( uint32_t channel )
{
  ...
  return GetRegisterSpecificBits( ADD_1ST_LGTE + 0x0100 * channel, 0, GetGateLongBitSize() );
}
...
void CAENDgtz::SetChannelPreGateNs( uint32_t channel, uint32_t value )
{
  ...
  if( value % boardInfo.nsPerSample == 0 )
    {
      SetRegisterSpecificBits( ADD_1ST_PGTE + 0x0100 * channel, 
			       0, 8, value / boardInfo.nsPerSample );
    }
  else
    {
      error = -3;
...
uint32_t CAENDgtz::GetChannelPreGateNs( uint32_t channel )
{
  ...
  return boardInfo.nsPerSample * GetRegisterSpecificBits( ADD_1ST_PGTE + 0x0100 * channel, 0, 8 );
}
...
void CAENDgtz::SetChannelShortGateNs( uint32_t channel, uint32_t value )
{
  ...
  if( value % boardInfo.nsPerSample == 0 )
    {
      SetRegisterSpecificBits( ADD_1ST_SGTE + 0x0100 * channel, 
			       0, GetGateShortBitSize(), value / boardInfo.nsPerSample );
    }
  else
    {
      error = -3;
...
uint32_t CAENDgtz::GetChannelShortGateNs( uint32_t channel )
{
  ...
  return boardInfo.nsPerSample * 
    GetRegisterSpecificBits( ADD_1ST_SGTE + 0x0100 * channel, 0, GetGateShortBitSize() );
}
...
void CAENDgtz::SetChannelLongGateNs( uint32_t channel, uint32_t value )
{
  ...
  if( value % boardInfo.nsPerSample == 0 )
    {
      SetRegisterSpecificBits( ADD_1ST_LGTE + 0x0100 * channel, 
			       0, GetGateLongBitSize(), value / boardInfo.nsPerSample );
    }
  else
    {
      error = -3;
...
uint32_t CAENDgtz::GetChannelLongGateNs( uint32_t channel )
{
  ...
  return boardInfo.nsPerSample * 
    GetRegisterSpecificBits( ADD_1ST_LGTE + 0x0100 * channel, 0, GetGateLongBitSize() );
}
...
void CAENDgtz::SetChannelPulsePolarity( uint32_t channel, uint32_t value )
{
  /*
    ...
    Invert Input: Individual setting for the input signal inversion.
  ...
  */
  if(value<2){
    SetRegisterSpecificBits(ADD_DPP_CTRL + 0x0100 * channel, 16,16, value, "SetChannelPulsePolarity");
  }else{
    error = -28;
    EmitError("SetChannelPulsePolarity");
  }
}
...
{
  return GetRegisterSpecificBits( ADD_DPP_CTRL + 0x0100*channel ,16, 16, "GetChannelPulsePolarity");
}
...
{
  if(value<4){
    SetRegisterSpecificBits( ADD_DPP_CTRL + 0x0100*channel, 18, 19, value, "SetChannelTriggerMode");
  }
  else{
    error = -3;
  }
  ...
  if( error != 0 ) {
    EmitError("SetChannelTriggerMode");
  }
}
...
{ 
  return GetRegisterSpecificBits( ADD_DPP_CTRL + 0x0100 * channel, 18, 19, "GetChannelTriggerMode" ); 
}
...
void CAENDgtz::SetChannelChargeSensitivity( uint32_t channel, uint32_t value )
{
  ...
  if( error != 0 ) 
    EmitError("SetChannelChargeSensitiivty");
}
...
void CAENDgtz::SetChannelDCFDDelay( int channel, int value )
{

  value /= boardInfo.nsPerSample; 
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_1ST_DCFD + (0x100 * channel), 
			     0, 7, value, "SetChannelDCFDDelay" );
  }
  else{
    error = -17;
    EmitError( "SetDCFDDelay" );
  }
}


uint32_t CAENDgtz::GetChannelDCFDDelay( int channel )
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_1ST_DCFD + (0x100 * channel), 
				   0, 7, "GetChannelDCFDDelay")*boardInfo.nsPerSample;
  } else {
    error = -17;
    EmitError("GetChannelDCFDDelay");
    return 0;
  }
}

void CAENDgtz::SetChannelDCFDFraction( int channel, int value )
{
  if(value>3){
    error = -28;
    EmitError("SetDCFDFraction");
    return;
  }
  ...
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_1ST_DCFD + (0x100 * channel), 
			     8, 9, value, "SetChannelDCFDFraction" );
  }
  else{
    error = -17;
    EmitError( "SetDCFDFraction" );
...
uint32_t CAENDgtz::GetChannelDCFDFraction(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_1ST_DCFD + (0x100 * channel), 
				   8, 9, "GetChannelDCFDFraction");
  } else {
    error = -17;
    EmitError("GetChannelDCFDFraction");
    return 0;
  }
}

void CAENDgtz::SetChannelDCFDInterpolation( int channel, int value )
{
  if(value>3){
    error = -28;
    EmitError("SetDCFDInterpolation");
    return;
  }
  ...
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_1ST_DCFD + (0x100 * channel), 
			     10, 11, value, "SetChannelDCFDInterpolation" );
  }
  else{
    error = -17;
    EmitError( "SetDCFDInterpolation" );
...
uint32_t CAENDgtz::GetChannelDCFDInterpolation(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_1ST_DCFD + (0x100 * channel), 
				   10,11, "GetChannelDCFDInterpolation");
  } else {
    error = -17;
    EmitError("GetChannelDCFDInterpolation");
    return 0;
  }
}


void CAENDgtz::SetChannelQthr( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_PSD_QTHR + (0x100 * channel), 
			     0, 15, value, "SetChannelQthr" );
  }
  else{
    error = -17;
    EmitError( "SetChannelQthr" );
  }
}


uint32_t CAENDgtz::GetChannelQthr(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_PSD_QTHR + (0x100 * channel), 
				   0, 15, "GetChannelQthr");
  } else {
    error = -17;
    EmitError("GetChannelQthr");
    return 0;
  }
}

void CAENDgtz::SetChannelEnabledQthr( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTRL + (0x100 * channel), 
			     25, 25, value, "SetChannelEnabledQthr" );
  }
  else{
    error = -17;
    EmitError( "SetChannelEnabledQthr" );
  }
}


uint32_t CAENDgtz::GetChannelEnabledQthr(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTRL + (0x100 * channel), 
				   25, 25, "GetChannelEnabledQthr");
  } else {
    error = -17;
    EmitError("GetChannelEnabledQthr");
    return 0;
  }
}
...
void CAENDgtz::SetChannelTriggerCounting( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTRL + (0x100 * channel), 
			     5, 5, value, "SetChannelTriggerCounting" );
  }
  else{
    error = -17;
    EmitError( "SetTriggerCounting" );
  }
}


uint32_t CAENDgtz::GetChannelTriggerCounting(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTRL + (0x100 * channel), 
				   5, 5 , "GetChannelTriggerCounting");
  } else {
    error = -17;
    EmitError("GetChannelTriggerCounting");
    return 0;
  }
}

void CAENDgtz::SetChannelDiscriminationMode( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTRL + (0x100 * channel), 
			     6, 6, value, "SetChannelDiscriminationMode" );
  }
  else{
    error = -17;
    EmitError( "SetDiscriminationMode" );
  }
}


uint32_t CAENDgtz::GetChannelDiscriminationMode(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTRL + (0x100 * channel), 
				   6, 6 , "GetChannelDiscriminationMode");
  } else {
    error = -17;
    EmitError("GetChannelDiscriminationMode");
    return 0;
  }
}

void CAENDgtz::SetChannelSignalSource( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTRL + (0x100 * channel), 
			     8, 8, value, "SetChannelSignalSource" );
  }
  else{
    error = -17;
    EmitError( "SetSignalSource" );
  }
}


uint32_t CAENDgtz::GetChannelSignalSource(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTRL + (0x100 * channel), 
				   8, 8 , "GetChannelSignalSource");
  } else {
    error = -17;
    EmitError("GetChannelSignalSource");
    return 0;
  }
}

void CAENDgtz::SetChannelInternalRate( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTRL + (0x100 * channel), 
			     9, 10, value, "SetChannelInternalRate" );
  }
  else{
    error = -17;
    EmitError( "SetSignalSource" );
...
uint32_t CAENDgtz::GetChannelInternalRate(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTRL + (0x100 * channel), 
				   9, 10, "GetChannelInternalRate");
  } else {
    error = -17;
    EmitError("GetChannelInternalRate");
    return 0;
  }
}

void CAENDgtz::SetChannelLocalTriggerMode( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTR2 + (0x100 * channel), 
			     0, 1, value, "SetChannelLocalTriggerMode" );
  }
  else{
    error = -17;
    EmitError( "SetChannelLocalTriggerMode" );
  }
}


uint32_t CAENDgtz::GetChannelLocalTriggerMode(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTR2 + (0x100 * channel), 
				   0, 1, "GetChannelLocalTriggerMode");
  } else {
    error = -17;
    EmitError("GetChannelLocalTriggerMode");
    return 0;
  }
}

void CAENDgtz::SetChannelEnabledLocalTrigger( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTR2 + (0x100 * channel), 
			     2, 2, value, "SetChannelEnabledLocalTrigger" );
  }
  else{
    error = -17;
    EmitError( "SetChannelEnabledLocalTrigger" );
  }
}


uint32_t CAENDgtz::GetChannelEnabledLocalTrigger(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTR2 + (0x100 * channel), 
				   2, 2, "GetChannelEnabledLocalTrigger");
  } else {
    error = -17;
    EmitError("GetChannelEnabledLocalTrigger");
    return 0;
  }
}


void CAENDgtz::SetChannelLocalValidation( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTR2 + (0x100 * channel), 
			     4, 5, value, "SetChannelLocalValidation" );
  }
  else{
    error = -17;
    EmitError( "SetChannelLocalValidation" );
  }
}


uint32_t CAENDgtz::GetChannelLocalValidation(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTR2 + (0x100 * channel), 
				   4, 5, "GetChannelLocalValidation");
  } else {
    error = -17;
    EmitError("GetChannelLocalValidation");
    return 0;
  }
}

void CAENDgtz::SetChannelEnabledLocalValidation( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTR2 + (0x100 * channel), 
			     6, 6, value, "SetChannelEnabledLocalValidation" );
  }
  else{
    error = -17;
    EmitError( "SetChannelEnabledLocalValidation" );
  }
}


uint32_t CAENDgtz::GetChannelEnabledLocalValidation(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTR2 + (0x100 * channel), 
				   6, 6, "GetChannelEnabledLocalValidation");
  } else {
    error = -17;
    EmitError("GetChannelEnabledLocalValidation");
    return 0;
  }
}

void CAENDgtz::SetChannelExtrasWord( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTR2 + (0x100 * channel), 
			     8, 10, value, "SetChannelExtrasWord" );
  }
  else{
    error = -17;
    EmitError( "SetChannelExtrasWord" );
  }
}


uint32_t CAENDgtz::GetChannelExtrasWord(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTR2 + (0x100 * channel), 
				   8, 10, "GetChannelExtrasWord");
  } else {
    error = -17;
    EmitError("GetChannelExtrasWord");
    return 0;
  }
}

void CAENDgtz::SetChannelEnabledSmoothing( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTR2 + (0x100 * channel), 
			     11, 11, value, "SetChannelEnabledSmoothing" );
  }
  else{
    error = -17;
    EmitError( "SetChannelEnabledSmoothing" );
  }
}


uint32_t CAENDgtz::GetChannelEnabledSmoothing(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTR2 + (0x100 * channel), 
				   11, 11, "GetChannelEnabledSmoothing");
  } else {
    error = -17;
    EmitError("GetChannelEnabledSmoothing");
    return 0;
  }
}


void CAENDgtz::SetChannelSmoothingFactor( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTR2 + (0x100 * channel), 
			     12, 15, value, "SetChannelSmoothingFactor" );
  }
  else{
    error = -17;
    EmitError( "SetChannelSmoothingFactor" );
  }
}


uint32_t CAENDgtz::GetChannelSmoothingFactor(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTR2 + (0x100 * channel), 
				   12, 15, "GetChannelSmoothingFactor");
  } else {
    error = -17;
    EmitError("GetChannelSmoothingFactor");
    return 0;
  }
}

void CAENDgtz::SetChannelVetoSource( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_DPP_CTR2 + (0x100 * channel), 
			     18, 19, value, "SetChannelVetoSource" );
  }
  else{
    error = -17;
    EmitError( "SetChannelVetoSource" );
  }
}


uint32_t CAENDgtz::GetChannelVetoSource(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_DPP_CTR2 + (0x100 * channel), 
				   18, 19, "GetChannelVetoSource");
  } else {
    error = -17;
    EmitError("GetChannelVetoSource");
    return 0;
  }
}

void CAENDgtz::SetChannelVetoWidth( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_1ST_VETO + (0x100 * channel), 
			     0, 15, value, "SetChannelVetoWidth" );
  }
  else{
    error = -17;
    EmitError( "SetChannelVetoWidth" );
  }
}


uint32_t CAENDgtz::GetChannelVetoWidth(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_1ST_VETO + (0x100 * channel), 
				   0, 15, "GetChannelVetoWidth");
  } else {
    error = -17;
    EmitError("GetChannelVetoWidth");
    return 0;
  }
}

void CAENDgtz::SetChannelVetoSteps( int channel, int value )
{
  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    SetRegisterSpecificBits( ADD_1ST_VETO + (0x100 * channel), 
			     16, 17, value, "SetChannelVetoSteps" );
  }
  else{
    error = -17;
    EmitError( "SetChannelVetoSteps" );
  }
}


uint32_t CAENDgtz::GetChannelVetoSteps(int channel)
{

  if( ( boardInfo.vers == 720 || boardInfo.vers == 730 ) 
      && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD ){
    return GetRegisterSpecificBits(ADD_1ST_VETO + (0x100 * channel), 
				   16, 17, "GetChannelVetoSteps");
  } else {
    error = -17;
    EmitError("GetChannelVetoSteps");
    return 0;
  }
}

/*
  The Trigger Hold-Off is a logic signal of programmable width generated by the trigger logic in correspondence of the
  fast discriminator output. Other triggers are inhibited for the overall Trigger Hold-Off duration.
...
void CAENDgtz::SetChannelTriggerHoldOff( int channel, int value )
{
  ...
  if( boardInfo.vers == 724 && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PHA )
    ...
    {
      // UM5678 - 725-730 DPP-PHA User Manual rev. 2
      SetRegisterSpecificBits( 0x1074 + 0x0100 * channel, 0, 15, value, "SetChannelTriggerHoldOff" );
      //SetRegisterSpecificBits( 0x1074 + 0x0100 * channel, 0, 9, value, "SetChannelTriggerHoldOff" );
    }
}
...
void CAENDgtz::SetChannelCoincMask( int channel, int mask )
{
  ...
  if( boardInfo.vers == 730 && boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PSD )
    {
      error = -7;
      EmitError();
    }
...
int CAENDgtz::GetChannelDynamicRange( int channel )
{
  ...
  if( boardInfo.vers == 730 || boardInfo.vers == 725 )
    {
      return GetRegisterSpecificBits( (channel == 255) ? (ADD_1ST_3VPP) : (ADD_1ST_3VPP + 0x0100 * channel), 0, 0,
				      "GetChannelDynamicRange" );
    }
  else if( boardInfo.vers == 724 )
    {
...
void CAENDgtz::SetChannelVetoWindow( uint32_t channel, uint32_t value )
{
  ...
  SetRegisterSpecificBits( ADD_1ST_VETO + 0x0100 * channel, 0, 16, value, "SetChannelVetoWindow" );
}
...
uint32_t CAENDgtz::GetChannelVetoWindow( uint32_t channel )
{
  ...
  return GetRegisterSpecificBits( ADD_1ST_VETO + 0x0100 * channel, 0, 16, "GetChannelVetoWindow" );
}
...
// --------------------------------------------------------------------------------
// START DPP-PHA specific parameters
// --------------------------------------------------------------------------------
...
/*
  ...
  In case of RC-CR2 this value must be equal (or 50% more) to the input rising edge, in such a way the RC-CR2 peak value
  corresponds to the height of the input signal.
  Address 0x1n58, 0x8058
  Mode R/W
  Attribute I

  the Input Rise Time is the time the RC-CR2 reaches its maximum value. This value should correspond
  to the input rise time, in such a way the RC-CR2 peak value corresponds to the height of the input signal.

  Time expressed in 16 ns step for 725 series and 8 ns for 730 series.
*/
...
void CAENDgtz::SetChannelDecayTime( uint32_t channel, uint32_t value )
{
  ...
  if( boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PHA )
	...
	case 725:
	case 730:
	  SetRegisterSpecificBits( 0x1068 + 0x0100 * channel, 
				   0, 15, value);
          break;
...
/*
  ...
  by the decimation. This has the analogous effect as reducing the sampling frequency of the
  board.
  NOTE: The decimation applies to the energy filter only (i.e. to Decay Time (Pole- Zero
  Compensation), Trap. Rise Time, and Trap. Flat Top). Timing filter (RC-CR2), Baseline, Trigger
  Hold-Off, and Record Length are not affected.
  Options are:
  00: Decimation disabled;
  01: 2 samples (125 MSps for 725, 250 MSps for 730);
  10: 4 samples (62.5 MSps for 725, 125 MSps for 730);
  11: 8 samples (31.25 MSps for 725, 62.5 MSps for 730).

  value: numero di samples, 2, 4, 8
*/
	...
	case 730:
	  switch( value )
	    ...
	    case 0:
	      ...
	      // 2 samples (125 MSps for 725, 250 MSps for 730)
	    case 2:
	      SetRegisterSpecificBits( 0x1080 + (0x0100 * channel), 8, 9, 1, "SetChannelDigitalGain" );
	      break;

	      // 4 samples (62.5 MSps for 725, 125 MSps for 730)
	    case 4:
	      SetRegisterSpecificBits( 0x1080 + (0x0100 * channel), 8, 9, 2, "SetChannelDigitalGain" );
	      break;

	      // 8 samples (31.25 MSps for 725, 62.5 MSps for 730)
	    case 8:
	      SetRegisterSpecificBits( 0x1080 + (0x0100 * channel), 8, 9, 3, "SetChannelDigitalGain" );
	      break;
  ...
  if( boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PHA )
	...
	case 730:
	    ...
	    switch( value )
	      {
	      case 0:
		decimation = 0;	// Decimation disabled
                break;

	      case 1:
		decimation = 2;	// 2 samples (125 MSps for 725, 250 MSps for 730)
                break;

	      case 2:
		decimation = 4;	// 4 samples (62.5 MSps for 725, 125 MSps for 730)
                break;

	      case 3:
		decimation = 8;	// 8 samples (31.25 MSps for 725, 62.5 MSps for 730)
                break;
...
/*
  ...
  resolution. Decimation gain is added to the trapezoid rescaling (bits[5:0]) and fine gain
  (register 0x1nC4).
  Options are:
  00: Digital Gain = 1;
  01: Digital Gain = 2 (only with decimation >= 2 samples);
  10: Digital Gain = 4 (only with decimation >= 4 samples);
  11: Digital Gain = 8 (only with decimation = 8 samples).

  value: digital gain, 1, 2, 4, 8
*/
...
/*
  Peak Mean: corresponds to the number of samples for the averaging window of the trapezoid
  height calculation.
...
void CAENDgtz::SetChannelPileUpFlagging( int channel, int value )
{
  ...
  */
  ...
  if( boardInfo.dppVersion == CAEN_DGTZ_DPPFirmware_PHA )
	...
	default:
	  error = -7;
	  EmitError( "SetChannelPileUpFlagging" );
          break;
	}
    }
  else
...
// --------------------------------------------------------------------------------
// END DPP-PHA Specific Parameters
// --------------------------------------------------------------------------------
  ...
  if( !config_file.good() )
    {
      ...
      EmitError();
      std::stringstream cmd; cmd << "ls -ltrh " << config_file_name;
      std::cout << system(cmd.str().c_str()) << std::endl;	
      return;
    }
...
void CAENDgtz::ConfigureFromJSON( const Json::Value &config )
{
  ...
  if( config.isMember("dgtzs") )
    {
	  ...
	  if( n == std::string::npos )	//std::string::npos (until end of string no "ch" member)
            {
	      ...
	      std::cout << "config[\"dgtzs\"][\"" << keyword << "\"]: '" << s_value << "', " << std::showbase << value << std::endl;
	      ...
	      if( keyword.compare( "DataAutoFlush" ) == 0 )
                {
		  SetDataAutoFlush( value );

		  if( value - GetDataAutoFlush() != 0 )
                    {
		      std::cout << "(!)  ^--- problem here" << std::endl;
		      error = -3;
		      EmitError();
	      ...
	      if( keyword.compare( "TriggerPropagation" ) == 0 )
                {
		  SetTriggerPropagation( value );

		  if( value - GetTriggerPropagation() != 0 )
                    {
		      std::cout << "(!)  ^--- problem here" << std::endl;
		      error = -3;
		      EmitError();
		  ...
		  */
		  ...
		  // channel: To be used only with digitizers running DPP-PSD and DPP-PHA firmware.-
		  //-This parameter is not supported in DPP-CI firmware.
		  // channel 255 is valid (255 = all channels)
		  // Note: for the DPP-PSD and DPP-PHA firmware of 730 family (also 725?), the Set function is managed inside-
		  //-couples of channels. It is only possible to set the record length for the even channel of the couple,-
		  //-then the same value is automatically applied for the odd channel of the couple.
		  ...
		  if( keyword.compare( "RecordLength" ) == 0 )
                    {
		      std::cout << "RecordLength, channel: " << channel << std::endl;
		  ...
		  if( keyword.compare( "SelfTrigger" ) == 0 )
		      ...
		      if( value - GetChannelSelfTrigger( channel ) != 0 )
                        {
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "DCFDDelay" ) == 0)
		    {
		      SetChannelDCFDDelay( channel, value);
			
		      if( value - GetChannelDCFDDelay( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "DCFDFraction" ) == 0)
		    {
		      SetChannelDCFDFraction( channel, value);
			
		      if( value - GetChannelDCFDFraction( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "DCFDInterpolation" ) == 0)
		    {
		      SetChannelDCFDInterpolation( channel, value);
			
		      if( value - GetChannelDCFDInterpolation( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "ChargeThreshold" ) == 0)
		    {
		      SetChannelQthr( channel, value);
			
		      if( value - GetChannelQthr( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "EnabledQthr" ) == 0)
		    {
		      SetChannelEnabledQthr( channel, value);
			
		      if( value - GetChannelEnabledQthr( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "TriggerCounting" ) == 0)
		    {
		      SetChannelTriggerCounting( channel, value);
			
		      if( value - GetChannelTriggerCounting( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "DiscriminationMode" ) == 0)
		    {
		      SetChannelDiscriminationMode( channel, value);
			
		      if( value - GetChannelDiscriminationMode( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "SignalSource" ) == 0)
		    {
		      SetChannelSignalSource( channel, value);
			
		      if( value - GetChannelSignalSource( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "InternalRate" ) == 0)
		    {
		      SetChannelInternalRate( channel, value);
			
		      if( value - GetChannelInternalRate( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "LocalTriggerLogic" ) == 0)
		    {
		      SetChannelLocalTriggerMode( channel, value);
			
		      if( value - GetChannelLocalTriggerMode( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "EnabledLocalTriggerLogic" ) == 0)
		    {
		      SetChannelEnabledLocalTrigger( channel, value);
			
		      if( value - GetChannelEnabledLocalTrigger( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "LocalValidationLogic" ) == 0)
		    {
		      SetChannelLocalValidation( channel, value);
		      
		      if( value - GetChannelLocalValidation( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "EnabledLocalValidationLogic" ) == 0)
		    {
		      SetChannelEnabledLocalValidation( channel, value);
			
		      if( value - GetChannelEnabledLocalValidation( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "ExtrasWord" ) == 0)
		    {
		      SetChannelExtrasWord( channel, value);
			
		      if( value - GetChannelExtrasWord( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "EnabledIntegrationSmoothing" ) == 0)
		    {
		      SetChannelEnabledSmoothing( channel, value);
			
		      if( value - GetChannelEnabledSmoothing( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "IntegrationSmoothingFactor" ) == 0)
		    {
		      SetChannelSmoothingFactor( channel, value);
			
		      if( value - GetChannelSmoothingFactor( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "VetoSource" ) == 0)
		    {
		      SetChannelVetoSource( channel, value);
			
		      if( value - GetChannelVetoSource( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "VetoWidth" ) == 0)
		    {
		      SetChannelVetoWidth( channel, value);
			
		      if( value - GetChannelVetoWidth( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  if( keyword.compare( "VetoSteps" ) == 0)
		    {
		      SetChannelVetoSteps( channel, value);
			
		      if( value - GetChannelVetoSteps( channel ) != 0 )
			{
			  std::cout << "(!)  ^--- problem here" << std::endl;
			  error = -3;
			  EmitError();
		  ...
		  // --------------------------------------------------------------------------------
		  // START DPP-PHA specific keywords
		  // --------------------------------------------------------------------------------
		  ...
		  if( keyword.compare( "DecayTime" ) == 0 )
                    {
		      std::cout << "nsPerTimetag = " << boardInfo.nsPerTimetag << std::endl;
		      value /= boardInfo.nsPerTimetag;
		  ...
		  // --------------------------------------------------------------------------------
		  // STOP DPP-PHA specific keywords
		  // --------------------------------------------------------------------------------
		  // --------------------------------------------------------------------------------
  ...
  if( config.isMember( "registers" ) ){
    ...
    const Json::Value registers_config = config[ "registers" ];    

    const Json::Value::Members registers_members = registers_config.getMemberNames();
    
    for( const std::string &member : registers_members )
      {

	if(member.rfind("reg_")==0)
	  {
 	    //std::cout << "Register description" << std::endl;
	    
	    const Json::Value reg_config = registers_config[member];

	    const Json::Value::Members reg_members = reg_config.getMemberNames();

	    for( const std::string &keyword : reg_members )
	      {
		std::string s_value;
		int value = 0;
		...
		try
		  {
		    s_value = reg_config[keyword].asString();
		  }
		catch (...) {}
		// We cannot use Json::LogicError to keep retrocompatibility with Ubuntu 14
		//catch (Json::LogicError &je) { std::cout << "JSON Logic error: " << je.what() << std::endl; }
		try
		  {
		    value = reg_config[keyword].asInt();
		  }
		catch (...) {}
		if( keyword.compare( "name" ) == 0 )
		  {
		    registerInfo.name = s_value;
		  }
		else if ( keyword.compare( "channel" ) == 0 )
		  {
		    registerInfo.channel=value;
		  }
		else if ( keyword.compare( "address" ) == 0 )
		  {
		    value = std::stoul(s_value,nullptr,0);
		    registerInfo.address = value;
		  }
		else if ( keyword.compare( "value" ) == 0 )
		  {
		    value = std::stoul(s_value,nullptr,0);
		    registerInfo.value = value;
		  }
		else if ( keyword.compare( "comment" ) == 0 )
		  {
		    //nothing to do, it is only to help the user
		  }
		else{
		  std::cerr << "Keyword not foreseen for the register description" << std::endl;
		}
	      }
	    //	    std::cout << "Setting register " << registerInfo.name << " for channel " << registerInfo.channel << std::endl;
	    CAEN_DGTZ_WriteRegister( handle, registerInfo.address , registerInfo.value);
	  }
