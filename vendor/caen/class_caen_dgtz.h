...
#ifndef MAXC_DG
// Max number of channels x digitizer
#define MAXC_DG 16
#endif

// Portable millisecond sleep. On Windows the Win32 Sleep(ms) already exists;
// elsewhere provide one with the same (milliseconds) semantics via usleep.
#if defined(_WIN32)
#  include <windows.h>
#else
#  include <unistd.h>
...
class CAENDgtz
{
 ...
 private:
  ...
  struct boardInfo_t {
    ...
    // Custom values
    uint16_t                    vers;
    CAEN_DGTZ_DPPFirmware_t     dppVersion;	// standard or DPP
 ...
 public:
  ...
  CAEN_DGTZ_DPPFirmware_t GetDPPVersion();
  ...
  void        SetChannelDCFDDelay( int channel, int value );

  uint32_t    GetChannelDCFDDelay( int channel );

  void        SetChannelDCFDFraction( int channel, int value );
    
  uint32_t    GetChannelDCFDFraction( int channel );

  void        SetChannelDCFDInterpolation( int channel, int value );
    
  uint32_t    GetChannelDCFDInterpolation( int channel );

  void        SetChannelQthr( int channel, int value );
    
  uint32_t    GetChannelQthr( int channel );

  void        SetChannelEnabledQthr( int channel, int value );
    
  uint32_t    GetChannelEnabledQthr( int channel );
    
  void        SetChannelTriggerCounting( int channel, int value );

  uint32_t    GetChannelTriggerCounting( int channel );

  void        SetChannelDiscriminationMode( int channel, int value );

  uint32_t    GetChannelDiscriminationMode( int channel );

  void        SetChannelSignalSource( int channel, int value );
    
  uint32_t    GetChannelSignalSource( int channel );
    
  void        SetChannelInternalRate( int channel, int value );
    
  uint32_t    GetChannelInternalRate( int channel );

  void        SetChannelLocalTriggerMode( int channel, int value );
  
  uint32_t    GetChannelLocalTriggerMode( int channel );
  
  void        SetChannelEnabledLocalTrigger( int channel, int value );
  
  uint32_t    GetChannelEnabledLocalTrigger( int channel );

  void        SetChannelLocalValidation( int channel, int value );
  
  uint32_t    GetChannelLocalValidation( int channel );

  void        SetChannelEnabledLocalValidation( int channel, int value );
  
  uint32_t    GetChannelEnabledLocalValidation( int channel );

  void        SetChannelExtrasWord( int channel, int value );
  
  uint32_t    GetChannelExtrasWord( int value );

  void        SetChannelEnabledSmoothing( int channel, int value );

  uint32_t    GetChannelEnabledSmoothing( int value );
  
  void        SetChannelSmoothingFactor( int channel, int value );

  uint32_t    GetChannelSmoothingFactor( int value);

  void        SetChannelVetoSource( int channel, int value );

  uint32_t    GetChannelVetoSource( int value );
  
  void        SetChannelVetoWidth( int channel, int value );
  
  uint32_t    GetChannelVetoWidth( int channel );

  void        SetChannelVetoSteps( int channel, int value );
  
  uint32_t    GetChannelVetoSteps( int channel );
    
  void        SetDataAutoFlush( int value );

  uint32_t    GetDataAutoFlush( );

  void        SetTriggerPropagation( int value );

  uint32_t    GetTriggerPropagation( );
